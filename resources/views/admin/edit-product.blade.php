        ...
        <div class="admin-flex-main">
            ...
            <main class="admin-main-content">
                ...
                <section class="admin-users-table-section" style="max-width:600px;margin:auto;">
                    ...
                    @endif
                    <form method="POST" action="{{ route('admin.products.update', $product) }}" class="admin-form" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div>
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" required class="admin-input" rows="4" style="resize:vertical;">{{ old('description', $product->description) }}</textarea>
                        </div>
                        <div>
                            <label for="price">Prix (DH)</label><br>
                            <input type="number" name="price" id="price" value="{{ old('price', $product->price) }}" step="0.01" min="0" required class="admin-input">
                        </div>
                        <div>
                            <label for="category">Catégorie</label><br>
                            <select name="category" id="category" required class="admin-input">
                                <option value="">Sélectionner une catégorie</option>
                                <option value="chat" {{ old('category', $product->category) == 'chat' ? 'selected' : '' }}>Chat</option>
                                <option value="chien" {{ old('category', $product->category) == 'chien' ? 'selected' : '' }}>Chien</option>
                                <option value="oiseau" {{ old('category', $product->category) == 'oiseau' ? 'selected' : '' }}>Oiseau</option>
                            </select>
                        </div>
                        <div>
                            ...
                            <div class="image-upload-container">
                                ...
                                <div class="image-preview" id="imagePreview">
                                    @if($product->image)
                                        <img src="{{ $product->image_url }}" alt="Image actuelle" class="preview-image">
